...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Sorted set: ra:lb:totalEarned (member = playerId, score = total earned)
  const raw = await redis.zrange(LB_TOTAL_EARNED, 0, limit - 1, { rev: true, withScores: true });
  ...
  if (Array.isArray(raw)) {
    ...
    if (raw.length > 0 && Array.isArray((raw as any)[0])) {
