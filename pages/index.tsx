...
import Expedition3D from '../components/Expedition3D'  // 3D expedition scene
import Shuffle from '../components/Shuffle'             // three-egg shuffle game
  ...
  return (
    <main className="max-w-5xl mx-auto p-6">
      ...
      <div className="flex gap-2 mb-4">
        <button
          className={'tab ' + (tab === 'tunnel' ? 'tab-active' : '')}
          onClick={() => setTab('tunnel')}
        >
          🐜 Ant Tunnel
        </button>
        <button
          className={'tab ' + (tab === 'hatch' ? 'tab-active' : '')}
          onClick={() => setTab('hatch')}
        >
          🥚 Queen&apos;s Egg Hatch
        </button>
        <button
          className={'tab ' + (tab === 'expedition' ? 'tab-active' : '')}
          onClick={() => setTab('expedition')}
        >
          🗺️ Expedition
        </button>
        <button
          className={'tab ' + (tab === 'shuffle' ? 'tab-active' : '')}
