...
export default function Shuffle() {
  ...
  // lanes: left positions in %
  const lanes = useMemo(() => [18, 50, 82], []);

  // always start clean when component mounts
  useEffect(() => {
    setPhase('idle');
    setCanPick(false);
    setProgress(0);
  ...
  const runShuffle = async () => {
    if (busy || phase === 'shuffling') return;
    setBusy(true);
    setCanPick(false);
    setPhase('shuffling');
    setProgress(0);

    // 20–28 swaps with ease-in-out timing for nicer motion
    const swaps = 20 + Math.floor(Math.random() * 9);
    for (let i = 0; i < swaps; i++) {
      ...
      // quadratic ease both ways
      const t = i / (swaps - 1);
      const eased = t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t;
      setProgress(Math.min(99, Math.floor(eased * 100)));

      // eslint-disable-next-line no-await-in-loop
      await wait(90 + Math.floor(180 * eased));
    }
  ...
  const onPick = async () => {
    ...
    setPhase('revealed');
    // soft reset so the CTA returns to Shuffle for the next round
    setTimeout(() => {
      setPhase('idle');
      setProgress(0);
    }, 900);
  ...
  return (
    <div className="ant-card">
      ...
      {/* WRAP keeps everything centered */}
      <div className="shuffle-wrap">
        {/* Scene */}
        <div className="shuffle-scene ant-scene">
          {/* decorative strip & rails */}
          <div className="strip" />
          <div className="rail rail-top" />
          <div className="rail rail-bottom" />

          {/* egg cards – positioned by lane % and centered via translateX(-50%) */}
          {order.map((eggId, pos) => (
            <button
              key={eggId}
        ...
        {/* Progress (under the scene, centered) */}
