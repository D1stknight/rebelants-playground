...
type Props = {
  layout: string[]; layouts?: string[][]; layoutIdx?: number; theme: Theme; themes?: Record<string, Theme>; cfg: Cfg; playing: boolean; faction?: string;
  onHud: (h: Hud) => void; onEnd: (r: EndResult) => void; onSfx: (n: string) => void; onQuit?: () => void;
  hudLine?: React.ReactNode;
};

export default function TunnelRun({ layout, layouts, layoutIdx = 0, theme, themes, cfg, playing, faction = "samurai", onHud, onEnd, onSfx, onQuit, hudLine }: Props) {
  ...
  useEffect(() => {
    ...
    const g = new Tunnel(cv, layout, theme, cfg, { onHud: (h) => cbRef.current.onHud(h), onEnd: (r) => cbRef.current.onEnd(r), onSfx: (n) => cbRef.current.onSfx(n) }, spritesRef.current, layouts, layoutIdx, themes);
